<?php 
    ...
    if ($data) {
        ...
        $sql = "SELECT t1.id, t1.correlativo, UPPER(CONCAT(t1.nombre, CONCAT(' ', CONCAT(t1.segundo_nombre, CONCAT(' ', CONCAT(t1.apellido, CONCAT(' ', t1.segundo_apellido))))))) as nombre, 
                t2.nombre as estado, t2.color, UPPER(t3.nombre) as campamento, UPPER(t1.direccion) as direccion, t1.zona, UPPER(t4.nombre) as colonia, t1.numero_contacto as telefono, UPPER(t1.otra_colonia) as otra_colonia, DATE_FORMAT(t1.created_at, '%d/%m/%Y %h:%i') as fecha_registro
                FROM paciente t1
